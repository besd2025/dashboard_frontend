  ...
  const navItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { name: "Liste", path: "/dashboard/cultivators/list" },
      ],
      ...
      subItems: [
        ...
        ...(user?.session?.category === "Admin"
          ? [
              {
                icon: <DetailIcon />,
                name: "Ajouter un Hangar",
                path: "/dashboard/hangars/add_new",
              },
              {
                icon: <DetailIcon />,
                name: "Ajouter un Collecteur",
                path: "/dashboard/hangars/add_collector",
              },
              {
                icon: <DetailIcon />,
                name: "Transfert",
                path: "/dashboard/hangars/hangars_transfert",
              },
            ]
          : []),
      ],
    ...
    },
    {
      ...
      subItems: [
        ...
        { name: "Ventes", path: "/dashboard/stocks/sold_out" },
        { name: "Sorties", path: "/dashboard/stocks/out/en_attente" },
      ],
  ...
  const renderMenuItems = (navItems, menuType) => (
    <ul className="flex flex-col gap-4">
      {navItems.map((nav, index) => (
        <li key={nav.path || `${nav.name}-${index}`}>
          {nav.subItems ? (
            <button
          ...
          {nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
            ...
            >
              <ul className="mt-2 space-y-1 ml-9  p-2 rounded">
                {nav.subItems.filter(Boolean).map((subItem, subIndex) => (
                  <li key={subItem.path || `${subItem.name}-${subIndex}`}>
                    <Link
                      href={subItem.path}
