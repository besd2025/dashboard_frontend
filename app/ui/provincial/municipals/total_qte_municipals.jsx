...
function TotalQtemunicipals() {
  const [data, setData] = useState([]);
  const [error, setError] = useState(null);
  useEffect(() => {
    async function getData() {
      try {
        const results = await fetchData("get", "municipals/total/", {
          params: {},
          additionalHeaders: {},
          body: {},
        });
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      <div className="flex items-center justify-c/enter  bg-gr/ay-100 rounded-xl dark:bg-gr/ay-800">
        ...
        <h4 className="ml-2 font-semibold text-gray-800 text-2xl dark:text-white/90">
          {data.total_municipals || 0} <span className="text-sm">KG</span>
        </h4>
      </div>
...
export default TotalQtemunicipals;
