...
import FilterHangarTransfertLists from "./filtre_hangar_transfer";
import EditHangarTransfer from "./edit_hangars_transfert";
function SearchHangarsList() {
  ...
  const [hangarName, setHangarName] = useState(null);
      ...
      try {
        ...
        if (filterData && Object.keys(filterData).length > 0) {
          results = await fetchData(
            "get",
            `/hangars/get_hangars_par_adresses/`,
            {
              params: {
                adress: filterData.zone,
              },
            }
          );
        } else {
        ...
        setData(results);
        console.log("Hangars data:", results);
  ...
  const updateHangar = (hangar_id) => {
    openModal();

    fetchData(
      "post",
      `hangars/get_change_hangar_for_transfert/`,

      {
        params: {},
        additionalHeaders: {},
        body: { hangar_id: hangar_id },
      }
    ).then((results) => {
    ...
    // setHangarId(hangar_id, hangar_name);
    // setHangarName(hangar_name);
  };
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03]  sm:px-6 sm:pt-6 ">
      <div className="flex flex-col lg:flex-row items-center justify-between w-full gap-2 px-3 py-3 border-b  border-gray-200 dark:border-gray-800 sm:gap-4  lg:border-b-0 lg:px-0 lg:py-4">
        ...
        <Search_Form onChange={(e) => setSearchData(e.target.value)} />
            ...
            >
              {data.map((order) => (
                <TableRow key={order.id}>
                  <TableCell className="px-0   py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <div className="relative inline-block ">
                      ...
                      >
                        <button
                          onClick={() =>
                            updateHangar(order.id, order.hangar_name)
                          }
                          tag="a"
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    {
                      order?.hangar_adress?.commune_code?.province_code
                        ?.province_name
                    }
                  </TableCell>
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    {order?.hangar_adress?.commune_code?.commune_name}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    {order?.hangar_adress?.zone_name}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      {/* <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[700px] m-4">
        <EditHangarTransfer hangar_id={hangarId} />
      </Modal> */}
    </div>
  );
}

export default SearchHangarsList;
